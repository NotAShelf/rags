...
> [!TIP]
> The global `Variable` is a function that returns an instance If you want to
> subclass this instead of `Service` import the class
>
> ```js
...
> [!CAUTION]
> using `setValue` will force a new value and thus signal `changed`, while
> setting `value` only signal if it gets a new value
>
> ```js
