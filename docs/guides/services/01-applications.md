...
- `query`: `(string) => App[]` takes a string and returns a list of apps that
...
- `app`:
  [Gio.DesktopAppInfo](https://gjs-docs.gnome.org/gio20~2.0/gio.desktopappinfo)
  the corresponding app info object
...
- `icon-name`: `string` the Icon entry in the corresponding .desktop file, can
  be a named icon or a path to an image
- `frequency`: 'number' number to take into consideration when sorting on
  `query` calls

### methods

- `launch`: `() => void` launches the app, you could also do
  `Utils.execAsync(['bash', '-c', app.executable])`, but the launch method keeps
  track of the frequency of launches which is used to sort the query
