...
- `getPlayer`: `(string) => Player` returns Player that has given string in its
  busName
...
- `position`: `(position: number)` this is signaled when the position is set
  explicitly
- `closed`
...
- `metadata`: `{ [key: string]: unknown }` metadata object, see
  [freedesktop.org](https://www.freedesktop.org/wiki/Specifications/mpris-spec/metadata/)
  for known keys
...
- `loop-status`: `"None" | "Track" | "Playlist" | null` null if unsupported by
  the player
