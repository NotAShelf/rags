...
/** Props for the Button widget, including click, hover, and scroll event handlers. */
 ...
 * @example
 * const btn = newButton({ label: 'Click me', on_clicked: (self) => print('clicked') });
 */
export function newButton<Child extends Gtk.Widget = Gtk.Widget, Attr = unknown>(
...
/** A clickable button with support for hover, scroll, and multi-button click events. */
export class Button<Child extends Gtk.Widget, Attr> extends Gtk.Button {
    ...
    constructor(props: ButtonProps<Child, Attr> = {} as ButtonProps<Child, Attr>, child?: Child) {
        ...
        this.connect('enter-notify-event', (_, event: Gdk.Event) => {
            if (this.isHovered(event)) return this.on_hover?.(this, event);
        });

        this.connect('leave-notify-event', (_, event: Gdk.Event) => {
            if (!this.isHovered(event)) return this.on_hover_lost?.(this, event);
        });
