...
/** A box with three slots: start, center, and end. */
    ...
    extends Gtk.Box
{
    ...
    constructor(
        props: CenterBoxProps<StartWidget, CenterWidget, EndWidget, Attr> = {} as CenterBoxProps<
            StartWidget,
            CenterWidget,
            EndWidget,
            Attr
    ...
    get children() {
        return [this.start_widget, this.center_widget, this.end_widget];
    }

    set children(children: [StartWidget | null, CenterWidget | null, EndWidget | null]) {
        const newChildren = children || [];

        newChildren.filter(ch => !newChildren?.includes(ch)).forEach(ch => ch && ch.destroy());

        if (children[0]) this.start_widget = children[0];

        if (children[1]) this.center_widget = children[1];

        if (children[2]) this.end_widget = children[2];
    }

    /** The widget packed at the start of the box. */
    get start_widget() {
        return this._get('start-widget') || (null as StartWidget | null);
    }

    set start_widget(child: StartWidget | null) {
        if (this.start_widget) this.start_widget.destroy();
    ...
    /** The widget packed at the end of the box. */
    get end_widget() {
        return this._get('end-widget') || (null as EndWidget | null);
    }

    set end_widget(child: EndWidget | null) {
        if (this.end_widget) this.end_widget.destroy();
    ...
    /** The widget placed at the center of the box. */
    get center_widget() {
        return this.get_center_widget() as CenterWidget | null;
    }
    ...
    /** Whether the center box lays out children vertically. */
    get vertical() {
    ...
    set vertical(vertical: boolean) {
        ...
        this.orientation = vertical ? Gtk.Orientation.VERTICAL : Gtk.Orientation.HORIZONTAL;
