...
/** GTK Stack wrapper for displaying one named child at a time with animated transitions. */
...
export class Stack<Children extends { [name: string]: Gtk.Widget }, Attr> extends Gtk.Stack {
    ...
    set children(children: Children) {
        ...
        for (const [name, widget] of Object.entries(children)) super.add_named(widget, name);
    ...
    set items(items: Array<[string, Gtk.Widget]>) {
        if (items) console.warn(Error('Stack.items is DEPRECATED, use Stack.children'));
    ...
    /** The transition animation type used when switching children. */
    get transition() {
